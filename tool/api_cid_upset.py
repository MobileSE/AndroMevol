...
def csv_generate(method_fields):
    ...
    repos = [repo for repo in total_repos if not repo.endswith('.csv') and not repo.endswith('.csv.bk')]
    repos.sort()
    device_outs = []
    device_headers = []
    device_apis = []
            ...
            if repo_apis:
                for repo in repos:
                    ...
                    for api in repo_apis[repo]:
                        ...
                        if 0 not in  device_curr:
                            print(device_curr)
                            all_exists += 1
                        if True:
                            device_apis.append(device_curr)
                            if device_curr[4] == 0:
                                if device_curr[2] == 1 and device_curr[3] == 1 and device_curr[5] == 1 and device_curr[6] == 1 and device_curr[7] == 1:
                                    print(device_curr)
    ...
    device_specific_out = os.path.join(out_base, 'device_upset_{}.csv'.format(method_field))
...
#    method_fields =  ['fields'] # ['methods'], ['fields']
