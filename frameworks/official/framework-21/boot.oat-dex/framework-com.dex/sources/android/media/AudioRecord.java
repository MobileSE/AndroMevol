...
public class AudioRecord {
    ...
    public AudioRecord(AudioAttributes attributes, AudioFormat format, int bufferSizeInBytes, int sessionId) throws IllegalArgumentException {
        ...
        if (attributes == null) {
            ...
            audioParamCheck(attributes.getCapturePreset(), rate, (format.getPropertySetMask() & 1) != 0 ? format.getEncoding() : 1);
    ...
    private void handleFullVolumeRec(boolean starting) {
        if (this.mIsSubmixFullVolume) {
            try {
                IAudioService.Stub.asInterface(ServiceManager.getService(Context.AUDIO_SERVICE)).forceRemoteSubmixFullVolume(starting, this.mICallBack);
