...
public final class StreamConfigurationMap {
    ...
    /* JADX INFO: Multiple debug info for r1v1 android.hardware.camera2.params.HighSpeedVideoConfiguration[]: [D('arr$' android.hardware.camera2.params.StreamConfiguration[]), D('arr$' android.hardware.camera2.params.HighSpeedVideoConfiguration[])] */
    ...
    private int checkArgumentFormatSupported(int format, boolean output) {
        int[] formats;
        checkArgumentFormat(format);
        for (int i : output ? getOutputFormats() : getInputFormats()) {
            if (format == i) {
                return format;
            }
        }
    ...
    private Size[] getInternalFormatSizes(int format, boolean output) {
        int sizeIndex;
        Integer sizesCount = getFormatsMap(output).get(Integer.valueOf(format));
        if (sizesCount == null) {
    ...
    private int[] getPublicFormats(boolean output) {
        ...
        int i = 0;
        for (Integer num : getFormatsMap(output).keySet()) {
