...
public class StackView extends AdapterViewAnimator {
    ...
    public class StackSlider {
        ...
        private float getDuration(boolean invert, float velocity) {
            ...
            float maxd = (float) Math.sqrt(Math.pow((double) StackView.this.mSlideAmount, 2.0d) + Math.pow((double) (0.4f * ((float) StackView.this.mSlideAmount)), 2.0d));
            if (velocity == 0.0f) {
                if (invert) {
                    f = 1.0f - (d / maxd);
                } else {
    ...
    @Override // android.widget.AdapterViewAnimator, android.view.View, android.widget.AdapterView
