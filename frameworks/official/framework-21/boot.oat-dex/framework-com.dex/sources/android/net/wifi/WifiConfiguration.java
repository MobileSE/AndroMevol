...
public class WifiConfiguration implements Parcelable {
    ...
    public static final Parcelable.Creator<WifiConfiguration> CREATOR = new Parcelable.Creator<WifiConfiguration>() {
        /* class android.net.wifi.WifiConfiguration.AnonymousClass2 */
        ...
        public WifiConfiguration createFromParcel(Parcel in) {
            ...
            config.autoJoinBailedDueToLowRssi = z;
            return config;
        }
    ...
    public final class Visibility {
        ...
        public String toString() {
            ...
            if (this.rssi24 > WifiConfiguration.INVALID_RSSI) {
                ...
                sbuf.append(PhoneConstants.APN_TYPE_ALL);
            }
            sbuf.append(" - ");
    ...
    public Visibility setVisibility(long age) {
        Visibility status2 = null;
        if (this.scanResultCache == null) {
            this.visibility = null;
        } else {
            status2 = new Visibility();
            ...
            for (ScanResult result : this.scanResultCache.values()) {
                if (result.seen != 0) {
                    ...
                    if (now_ms - result.seen <= age) {
                        if (result.is5GHz()) {
                            if (result.level > status2.rssi5) {
                                status2.rssi5 = result.level;
                                status2.age5 = result.seen;
                                status2.BSSID5 = result.BSSID;
                            }
                        } else if (result.is24GHz() && result.level > status2.rssi24) {
                            status2.rssi24 = result.level;
                            status2.age24 = result.seen;
                            status2.BSSID24 = result.BSSID;
                        }
                    }
                }
            }
            this.visibility = status2;
        }
        return status2;
    }
    ...
    public void setAutoJoinStatus(int status2) {
        if (status2 < 0) {
            status2 = 0;
        }
        if (status2 == 0) {
            this.blackListTimestamp = 0;
        } else if (status2 > this.autoJoinStatus) {
            this.blackListTimestamp = System.currentTimeMillis();
        }
        if (status2 != this.autoJoinStatus) {
            this.autoJoinStatus = status2;
            this.dirty = true;
        }
    }
    ...
    public String toString() {
        ...
        if (this.connectChoices != null) {
            for (String key2 : this.connectChoices.keySet()) {
                Integer choice = this.connectChoices.get(key2);
                if (choice != null) {
