...
public final class ScanRecord {
    ...
    public static ScanRecord parseFromBytes(byte[] scanRecord) {
        ...
        while (true) {
            try {
                if (currentPos < scanRecord.length) {
                    ...
                    try {
                        int length = scanRecord[currentPos] & BatteryStats.HistoryItem.CMD_NULL;
                        if (length != 0) {
                            int dataLength = length - 1;
                            int currentPos3 = currentPos2 + 1;
                            switch (scanRecord[currentPos2] & BatteryStats.HistoryItem.CMD_NULL) {
                                case 1:
                                ...
                                case 9:
                                    ...
                                    break;
                                case 10:
                                    b = scanRecord[currentPos3];
                                    break;
                                case 22:
                                    serviceData.put(BluetoothUuid.parseUuidFrom(extractBytes(scanRecord, currentPos3, 2)), extractBytes(scanRecord, currentPos3 + 2, dataLength - 2));
                                    break;
                                case 255:
                                    manufacturerData.put(((scanRecord[currentPos3 + 1] & BatteryStats.HistoryItem.CMD_NULL) << 8) + (scanRecord[currentPos3] & BatteryStats.HistoryItem.CMD_NULL), extractBytes(scanRecord, currentPos3 + 2, dataLength - 2));
                                    break;
                            }
        ...
        return new ScanRecord(serviceUuids, manufacturerData, serviceData, advertiseFlag, b, localName, scanRecord);
    }
