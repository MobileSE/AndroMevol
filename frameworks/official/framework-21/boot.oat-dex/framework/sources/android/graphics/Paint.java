...
public class Paint {
    ...
    public float measureText(CharSequence text, int start, int end) {
        if (text == null) {
            ...
            float measureText = measureText(buf, 0, end - start);
    ...
    public int breakText(char[] text, int index, int count, float maxWidth, float[] measuredWidth) {
        if (text == null) {
            ...
            float oldSize = getTextSize();
            setTextSize(this.mCompatScaling * oldSize);
            int native_breakText = native_breakText(this.mNativePaint, this.mNativeTypeface, text, index, count, maxWidth * this.mCompatScaling, this.mBidiFlags, measuredWidth);
            setTextSize(oldSize);
    ...
    public int breakText(String text, boolean measureForwards, float maxWidth, float[] measuredWidth) {
        if (text == null) {
            ...
            float oldSize = getTextSize();
            setTextSize(this.mCompatScaling * oldSize);
            int native_breakText = native_breakText(this.mNativePaint, this.mNativeTypeface, text, measureForwards, maxWidth * this.mCompatScaling, this.mBidiFlags, measuredWidth);
            setTextSize(oldSize);
            if (measuredWidth == null) {
                return native_breakText;
            }
    ...
    public float getTextRunAdvances(CharSequence text, int start, int end, int contextStart, int contextEnd, boolean isRtl, float[] advances, int advancesIndex) {
        ...
        if (((advances == null ? 0 : (advances.length - advancesIndex) - (end - start)) | (text.length() - contextEnd) | start | end | contextStart | contextEnd | advancesIndex | (end - start) | (start - contextStart) | (contextEnd - end)) < 0) {
            ...
            float textRunAdvances = getTextRunAdvances(buf, start - contextStart, end - start, 0, contextLen, isRtl, advances, advancesIndex);
            TemporaryBuffer.recycle(buf);
