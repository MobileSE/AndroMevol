...
public abstract class Visibility extends Transition {
    ...
    private static final String[] sTransitionProperties = {PROPNAME_VISIBILITY};
        ...
        if (overlayView != null) {
            ...
            if (animator2 != null) {
                animator2.addListener(new AnimatorListenerAdapter() {
                    /* class android.transition.Visibility.AnonymousClass2 */
                    boolean mCanceled = false;

                    public void onAnimationPause(Animator animation) {
                        if (!this.mCanceled && !isForcedVisibility) {
                            viewToKeep.setVisibility(endVisibility);
                        }
                    }
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (isForcedVisibility) {
                            viewToKeep.setTransitionAlpha(0.0f);
                        } else {
                            viewToKeep.setVisibility(endVisibility);
                        }
                    }
    ...
    @Override // android.transition.Transition
    public boolean areValuesChanged(TransitionValues oldValues, TransitionValues newValues) {
        VisibilityInfo changeInfo = getVisibilityChangeInfo(oldValues, newValues);
        if ((oldValues == null && newValues == null) || !changeInfo.visibilityChange) {
            return false;
        }
