...
public class Intent implements Parcelable, Cloneable {
    ...
    public static Intent parseUri(String uri, int flags) throws URISyntaxException {
        ...
        String scheme = null;
        int i2 = i + "#Intent;".length();
        while (!uri.startsWith("end", i2)) {
            int eq = uri.indexOf(61, i2);
            if (eq < 0) {
                eq = i2 - 1;
            }
            int semi = uri.indexOf(59, i2);
            String value = eq < semi ? Uri.decode(uri.substring(eq + 1, semi)) : ProxyInfo.LOCAL_EXCL_LIST;
            if (uri.startsWith("action=", i2)) {
                intent2.setAction(value);
            } else if (uri.startsWith("category=", i2)) {
                intent2.addCategory(value);
            } else if (uri.startsWith("type=", i2)) {
                intent2.mType = value;
            } else if (uri.startsWith("launchFlags=", i2)) {
                intent2.mFlags = Integer.decode(value).intValue();
            } else if (uri.startsWith("package=", i2)) {
                intent2.mPackage = value;
            } else if (uri.startsWith("component=", i2)) {
                intent2.mComponent = ComponentName.unflattenFromString(value);
            } else if (uri.startsWith("scheme=", i2)) {
                scheme = value;
            } else if (uri.startsWith("sourceBounds=", i2)) {
                intent2.mSourceBounds = Rect.unflattenFromString(value);
            } else if (semi != i2 + 3 || !uri.startsWith("SEL", i2)) {
                String key = Uri.decode(uri.substring(i2 + 2, eq));
                if (intent2.mExtras == null) {
                    intent2.mExtras = new Bundle();
                }
                Bundle b = intent2.mExtras;
                if (uri.startsWith("S.", i2)) {
                    b.putString(key, value);
                } else if (uri.startsWith("B.", i2)) {
                    b.putBoolean(key, Boolean.parseBoolean(value));
                } else if (uri.startsWith("b.", i2)) {
                    b.putByte(key, Byte.parseByte(value));
                } else if (uri.startsWith("c.", i2)) {
                    b.putChar(key, value.charAt(0));
                } else if (uri.startsWith("d.", i2)) {
                    b.putDouble(key, Double.parseDouble(value));
                } else if (uri.startsWith("f.", i2)) {
                    b.putFloat(key, Float.parseFloat(value));
                } else if (uri.startsWith("i.", i2)) {
                    b.putInt(key, Integer.parseInt(value));
                } else if (uri.startsWith("l.", i2)) {
                    b.putLong(key, Long.parseLong(value));
                } else if (uri.startsWith("s.", i2)) {
                    b.putShort(key, Short.parseShort(value));
                } else {
                    throw new URISyntaxException(uri, "unknown EXTRA type", i2);
                }
            } else {
                intent2 = new Intent();
            }
    ...
    public static Intent getIntentOld(String uri) throws URISyntaxException {
        ...
        if (uri.regionMatches(i2, "component(", 0, 10)) {
            ...
            if (sep2 >= 0 && sep2 < j5) {
                intent.mComponent = new ComponentName(uri.substring(i7, sep2), uri.substring(sep2 + 1, j5));
            }
            i2 = j5 + 1;
        }
                ...
                switch (type) {
                    ...
                    default:
                        try {
                            ...
                            throw new URISyntaxException(uri, "EXTRA value can't be parsed", j7);
                        }
                }
    ...
    public Object getExtra(String name, Object defaultValue) {
        Object result2;
        return (this.mExtras == null || (result2 = this.mExtras.get(name)) == null) ? defaultValue : result2;
    }
        ...
        if (this.mExtras != null) {
            for (String key : this.mExtras.keySet()) {
                ...
                if (value instanceof String) {
                    ...
                    entryType = value instanceof Boolean ? 'B' : value instanceof Byte ? 'b' : value instanceof Character ? 'c' : value instanceof Double ? DateFormat.DATE : value instanceof Float ? 'f' : value instanceof Integer ? 'i' : value instanceof Long ? 'l' : value instanceof Short ? 's' : 0;
                }
                if (entryType != 0) {
    ...
    private static ClipData.Item makeClipItem(ArrayList<Uri> streams, ArrayList<CharSequence> texts, ArrayList<String> htmlTexts, int which) {
        ...
        String htmlText;
        if (streams != null) {
            uri = streams.get(which);
        } else {
            uri = null;
        }
        if (texts != null) {
            text = texts.get(which);
        } else {
            text = null;
        }
        if (htmlTexts != null) {
            htmlText = htmlTexts.get(which);
        } else {
            htmlText = null;
        }
