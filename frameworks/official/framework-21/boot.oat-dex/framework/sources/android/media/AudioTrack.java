...
public class AudioTrack {
    ...
    public AudioTrack(AudioAttributes attributes, AudioFormat format, int bufferSizeInBytes, int mode, int sessionId) throws IllegalArgumentException {
        ...
        if (attributes == null) {
            ...
            audioParamCheck(rate, (format.getPropertySetMask() & 4) != 0 ? format.getChannelMask() : 12, (format.getPropertySetMask() & 1) != 0 ? format.getEncoding() : 1, mode);
            this.mStreamType = -1;
    ...
    public int write(ByteBuffer audioData, int sizeInBytes, int writeMode) {
        ...
        if (this.mState == 0) {
            ...
            if (audioData.isDirect()) {
                ...
                int position2 = audioData.position() + NioUtils.unsafeArrayOffset(audioData);
                int i2 = this.mAudioFormat;
                if (writeMode == 0) {
                    z = true;
                }
                ret = native_write_byte(unsafeArray, position2, sizeInBytes, i2, z);
            }
            if (this.mDataLoadMode == 0 && this.mState == 2 && ret > 0) {
                this.mState = 1;
            }
        ...
        NativeEventHandlerDelegate(final AudioTrack track, final OnPlaybackPositionUpdateListener listener, Handler handler) {
            ...
            if (looper != null) {
                this.mHandler = new Handler(looper, AudioTrack.this) {
                    /* class android.media.AudioTrack.NativeEventHandlerDelegate.AnonymousClass1 */
