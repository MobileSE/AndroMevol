...
class Utils {
    ...
    static Range<Integer> factorRange(Range<Integer> range, int factor) {
        return factor == 1 ? range : Range.create(Integer.valueOf(divUp(range.getLower().intValue(), factor)), Integer.valueOf(range.getUpper().intValue() / factor));
    }

    static Range<Long> factorRange(Range<Long> range, long factor) {
        return factor == 1 ? range : Range.create(Long.valueOf(divUp(range.getLower().longValue(), factor)), Long.valueOf(range.getUpper().longValue() / factor));
    }
    ...
    static Range<Rational> scaleRange(Range<Rational> range, int num, int den) {
        return num == den ? range : Range.create(scaleRatio(range.getLower(), num, den), scaleRatio(range.getUpper(), num, den));
    }
