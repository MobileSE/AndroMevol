...
public class PointCloud {
    ...
    public PointCloud(Drawable drawable) {
        ...
        this.mPaint.setColor(Color.rgb((int) R.styleable.Theme_actionBarTheme, (int) R.styleable.Theme_actionBarTheme, (int) R.styleable.Theme_actionBarTheme));
    ...
    public int getAlphaForPoint(Point point) {
        ...
        if (glowDistance < this.glowManager.radius) {
            glowAlpha = this.glowManager.alpha * max(0.0f, (float) Math.pow((double) FloatMath.cos((0.7853982f * glowDistance) / this.glowManager.radius), 10.0d));
        }
        float radius = hypot(point.x, point.y);
        float waveAlpha = 0.0f;
        if (radius < this.waveManager.radius * MIN_POINT_SIZE) {
            waveAlpha = this.waveManager.alpha * max(0.0f, (float) Math.pow((double) FloatMath.cos((1.5707964f * (radius - this.waveManager.radius)) / this.waveManager.radius), 6.0d));
        }
