...
public class LockPatternUtils {
    ...
    public int getActivePasswordQuality() {
        switch ((int) getLong(PASSWORD_TYPE_KEY, 65536)) {
            case 32768:
                ...
                return 0;
            case Protocol.BASE_SYSTEM_RESERVED:
                if (isLockPatternEnabled()) {
                    return Protocol.BASE_SYSTEM_RESERVED;
                }
                return 0;
            case Protocol.BASE_WIFI:
                if (isLockPasswordEnabled()) {
                    return Protocol.BASE_WIFI;
                }
                return 0;
            case Protocol.BASE_DHCP:
                if (isLockPasswordEnabled()) {
                    return Protocol.BASE_DHCP;
                }
                return 0;
            case Protocol.BASE_DATA_CONNECTION:
                if (isLockPasswordEnabled()) {
                    return Protocol.BASE_DATA_CONNECTION;
                }
                return 0;
            case Protocol.BASE_DNS_PINGER:
                if (isLockPasswordEnabled()) {
                    return Protocol.BASE_DNS_PINGER;
                }
                return 0;
            case Protocol.BASE_NSD_MANAGER:
                if (isLockPasswordEnabled()) {
    ...
    public void clearLock(boolean isFallback) {
        ...
        saveLockPassword(null, Protocol.BASE_SYSTEM_RESERVED);
    ...
    public void saveLockPattern(List<LockPatternView.Cell> pattern, boolean isFallback) {
        try {
            ...
            if (pattern != null) {
                ...
                if (!isFallback) {
                    deleteGallery();
                    setLong(PASSWORD_TYPE_KEY, 65536);
                    dpm.setActivePasswordState(Protocol.BASE_SYSTEM_RESERVED, pattern.size(), 0, 0, 0, 0, 0, 0, userId);
                } else {
                    setLong(PASSWORD_TYPE_KEY, 32768);
                    setLong(PASSWORD_TYPE_ALTERNATE_KEY, 65536);
                    finishBiometricWeak();
    ...
    public static int computePasswordQuality(String password) {
        ...
        if (hasDigit) {
            return maxLengthSequence(password) > 3 ? Protocol.BASE_WIFI : Protocol.BASE_DHCP;
        }
        return 0;
    }
    ...
    public boolean isLockPatternEnabled() {
        ...
        if (getLong(PASSWORD_TYPE_ALTERNATE_KEY, 0) == 65536) {
            backupEnabled = true;
        } else {
            backupEnabled = false;
        }
        if (getBoolean("lock_pattern_autolock", false)) {
            if (getLong(PASSWORD_TYPE_KEY, 0) == 65536) {
                return true;
            }
    ...
    public boolean isBiometricWeakLivelinessEnabled() {
        return (1 & getLong("lock_biometric_weak_flags", 0)) != 0;
    }
    ...
    private void writeAppWidgets(int[] appWidgetIds) {
        Settings.Secure.putStringForUser(this.mContentResolver, "lock_screen_appwidget_ids", combineStrings(appWidgetIds, ","), -2);
    }
