...
public class PointerLocationView extends View implements InputManager.InputDeviceListener, WindowManagerPolicy.PointerEventListener {
    ...
    public PointerLocationView(Context c) {
        ...
        this.mTextPaint.setARGB(R.styleable.Theme_actionBarTheme, 0, 0, 0);
        this.mTextBackgroundPaint = new Paint();
        this.mTextBackgroundPaint.setAntiAlias(false);
        this.mTextBackgroundPaint.setARGB(128, R.styleable.Theme_actionBarTheme, R.styleable.Theme_actionBarTheme, R.styleable.Theme_actionBarTheme);
        this.mTextLevelPaint = new Paint();
        this.mTextLevelPaint.setAntiAlias(false);
        this.mTextLevelPaint.setARGB(R.styleable.Theme_textEditSuggestionItemLayout, R.styleable.Theme_actionBarTheme, 0, 0);
        this.mPaint = new Paint();
        this.mPaint.setAntiAlias(true);
        this.mPaint.setARGB(R.styleable.Theme_actionBarTheme, R.styleable.Theme_actionBarTheme, R.styleable.Theme_actionBarTheme, R.styleable.Theme_actionBarTheme);
        this.mPaint.setStyle(Paint.Style.STROKE);
        this.mPaint.setStrokeWidth(2.0f);
        this.mCurrentPointPaint = new Paint();
        this.mCurrentPointPaint.setAntiAlias(true);
        this.mCurrentPointPaint.setARGB(R.styleable.Theme_actionBarTheme, R.styleable.Theme_actionBarTheme, 0, 0);
        ...
        this.mTargetPaint.setARGB(R.styleable.Theme_actionBarTheme, 0, 0, R.styleable.Theme_textEditSuggestionItemLayout);
        this.mPathPaint = new Paint();
        this.mPathPaint.setAntiAlias(false);
        this.mPathPaint.setARGB(R.styleable.Theme_actionBarTheme, 0, 96, R.styleable.Theme_actionBarTheme);
        this.mPaint.setStyle(Paint.Style.STROKE);
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int p = 0; p < NP; p++) {
            ...
            boolean drawn = false;
            this.mPaint.setARGB(R.styleable.Theme_actionBarTheme, 128, R.styleable.Theme_actionBarTheme, R.styleable.Theme_actionBarTheme);
            for (int i = 0; i < N2; i++) {
                ...
                if (Float.isNaN(x)) {
                    ...
                    if (haveLast) {
                        ...
                        canvas.drawPoint(lastX, lastY, ps2.mTraceCurrent[i] ? this.mCurrentPointPaint : this.mPaint);
                        drawn = true;
                    }
                    lastX = x;
                    lastY = y;
            ...
            if (drawn) {
                ...
                for (int i2 = -3; i2 <= 2; i2++) {
                    float x2 = ps2.mEstimator.estimateX(((float) i2) * 0.02f);
                    float y2 = ps2.mEstimator.estimateY(((float) i2) * 0.02f);
                    canvas.drawLine(lx, ly, x2, y2, this.mPaint);
                    lx = x2;
                    ly = y2;
                }
                this.mPaint.setARGB(R.styleable.Theme_actionBarTheme, R.styleable.Theme_actionBarTheme, 64, 128);
                canvas.drawLine(lastX, lastY, lastX + (ps2.mXVelocity * 16.0f), lastY + (ps2.mYVelocity * 16.0f), this.mPaint);
                if (this.mAltVelocity != null) {
                    ...
                    for (int i3 = -3; i3 <= 2; i3++) {
                        float x3 = ps2.mAltEstimator.estimateX(((float) i3) * 0.02f);
                        float y3 = ps2.mAltEstimator.estimateY(((float) i3) * 0.02f);
                        canvas.drawLine(lx2, ly2, x3, y3, this.mPaint);
                        lx2 = x3;
                        ly2 = y3;
                    }
                    this.mPaint.setARGB(R.styleable.Theme_actionBarTheme, 64, R.styleable.Theme_actionBarTheme, 128);
                    canvas.drawLine(lastX, lastY, lastX + (ps2.mAltXVelocity * 16.0f), lastY + (ps2.mAltYVelocity * 16.0f), this.mPaint);
                }
            }
            if (this.mCurDown && ps2.mCurDown) {
                ...
                this.mPaint.setARGB(R.styleable.Theme_actionBarTheme, pressureLevel, R.styleable.Theme_actionBarTheme, 255 - pressureLevel);
                canvas.drawPoint(ps2.mCoords.x, ps2.mCoords.y, this.mPaint);
                this.mPaint.setARGB(R.styleable.Theme_actionBarTheme, pressureLevel, 255 - pressureLevel, 128);
                ...
                this.mPaint.setARGB(R.styleable.Theme_actionBarTheme, pressureLevel, 128, 255 - pressureLevel);
                ...
                this.mPaint.setARGB(R.styleable.Theme_actionBarTheme, pressureLevel, R.styleable.Theme_actionBarTheme, 0);
                float orientationVectorX = (float) (Math.sin((double) ps2.mCoords.orientation) * ((double) arrowSize));
                float orientationVectorY = (float) ((-Math.cos((double) ps2.mCoords.orientation)) * ((double) arrowSize));
    ...
    private void logCoords(String type, int action, int index, MotionEvent.PointerCoords coords, int id, MotionEvent event) {
        ...
        switch (action & R.styleable.Theme_actionBarTheme) {
            case 0:
                prefix = "DOWN";
                break;
            case 1:
    ...
    public void onPointerEvent(MotionEvent event) {
        ...
        int action = event.getAction();
        int NP = this.mPointers.size();
        if (action == 0 || (action & R.styleable.Theme_actionBarTheme) == 5) {
        ...
        for (int historyPos = 0; historyPos < N; historyPos++) {
            for (int i = 0; i < NI; i++) {
                ...
                MotionEvent.PointerCoords coords2 = ps3 != null ? ps3.mCoords : this.mTempCoords;
        ...
        if (action == 1 || action == 3 || (action & R.styleable.Theme_actionBarTheme) == 6) {
