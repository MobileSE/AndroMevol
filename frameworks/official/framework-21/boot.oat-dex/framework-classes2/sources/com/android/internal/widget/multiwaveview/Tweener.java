...
class Tweener {
    ...
    private static Animator.AnimatorListener mCleanupListener = new AnimatorListenerAdapter() {
        /* class com.android.internal.widget.multiwaveview.Tweener.AnonymousClass1 */
    ...
    public static void remove(Animator animator2) {
        Iterator<Map.Entry<Object, Tweener>> iter = sTweens.entrySet().iterator();
        while (iter.hasNext()) {
            if (iter.next().getValue().animator == animator2) {
                iter.remove();
                return;
            }
        }
        ...
        for (int i = 0; i < vars.length; i += 2) {
            ...
            if (!"simultaneousTween".equals(key)) {
                if ("ease".equals(key)) {
                    ...
                    props.add(PropertyValuesHolder.ofFloat(key, ((Number) value).floatValue()));
                } else {
    ...
    public Tweener from(Object object, long duration, Object... vars) {
