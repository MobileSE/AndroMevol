...
public class GlowPadView extends View {
    ...
    public GlowPadView(Context context, AttributeSet attrs) {
        ...
        this.mResetListener = new AnimatorListenerAdapter() {
            /* class com.android.internal.widget.multiwaveview.GlowPadView.AnonymousClass1 */
        ...
        this.mResetListenerWithPing = new AnimatorListenerAdapter() {
            /* class com.android.internal.widget.multiwaveview.GlowPadView.AnonymousClass2 */
        ...
        this.mUpdateListener = new ValueAnimator.AnimatorUpdateListener() {
            /* class com.android.internal.widget.multiwaveview.GlowPadView.AnonymousClass3 */
        ...
        this.mTargetUpdateListener = new AnimatorListenerAdapter() {
            /* class com.android.internal.widget.multiwaveview.GlowPadView.AnonymousClass4 */
        ...
        this.mFirstItemOffset = (float) Math.toRadians((double) a.getFloat(14, (float) Math.toDegrees((double) this.mFirstItemOffset)));
    ...
    private void switchToState(int state, float x, float y) {
        switch (state) {
            ...
            case 2:
                ...
                deactivateTargets();
                showTargets(true);
                startBackgroundAnimation(R.styleable.Theme_colorActivatedHighlight, 1.0f);
                setGrabbedState(1);
    ...
    private void doFinish() {
        ...
        if (targetHit) {
            ...
            hideGlow(R.styleable.Theme_colorActivatedHighlight, RETURN_TO_HOME_DELAY, 0.0f, this.mResetListener);
            dispatchTriggerEvent(activeTarget);
            if (!this.mAlwaysTrackFinger) {
                this.mTargetAnimations.stop();
            }
        } else {
            hideGlow(R.styleable.Theme_colorActivatedHighlight, 0, 0.0f, this.mResetListenerWithPing);
    ...
    private void hideTargets(boolean animate, boolean expanded) {
        ...
        int duration = animate ? R.styleable.Theme_colorActivatedHighlight : 0;
        int delay = animate ? R.styleable.Theme_colorActivatedHighlight : 0;
    ...
    private void vibrate() {
        ...
        if (this.mVibrator != null && hapticEnabled) {
            this.mVibrator.vibrate((long) this.mVibrationDuration, VIBRATION_ATTRIBUTES);
        }
    }
    ...
    private ArrayList<TargetDrawable> loadDrawableArray(int resourceId) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            drawables.add(new TargetDrawable(res, value != null ? value.resourceId : 0));
        }
        array.recycle();
        return drawables;
    }
    ...
    public void ping() {
        if (this.mFeedbackCount > 0) {
            ...
            if (waveAnimations.size() > 0 && ((Tweener) waveAnimations.get(0)).animator.isRunning() && ((Tweener) waveAnimations.get(0)).animator.getCurrentPlayTime() < 500) {
    ...
    private void startWaveAnimation() {
        ...
        this.mWaveAnimations.add(Tweener.to(this.mPointCloud.waveManager, 1000, "ease", Ease.Quad.easeOut, "delay", 0, "radius", Float.valueOf(this.mOuterRadius * 2.0f), "onUpdate", this.mUpdateListener, "onComplete", new AnimatorListenerAdapter() {
            /* class com.android.internal.widget.multiwaveview.GlowPadView.AnonymousClass5 */
    ...
    private void startBackgroundAnimation(int duration, float alpha) {
        ...
        if (this.mAlwaysTrackFinger && background != null) {
            ...
            this.mBackgroundAnimator = Tweener.to(background, (long) duration, "ease", Ease.Cubic.easeIn, "alpha", Integer.valueOf((int) (255.0f * alpha)), "delay", 50);
    ...
    private void handleMove(MotionEvent event) {
        ...
        if (actionIndex != -1) {
            ...
            while (k < historySize + 1) {
                ...
                float touchRadius = (float) Math.sqrt((double) dist2(tx, ty));
                ...
                if (this.mDragging) {
                    ...
                    for (int i = 0; i < ntargets; i++) {
                        ...
                        if (targets.get(i).isEnabled()) {
                            if (((angleRad > targetMinRad && angleRad <= targetMaxRad) || (6.283185307179586d + angleRad > targetMinRad && 6.283185307179586d + angleRad <= targetMaxRad) || (angleRad - 6.283185307179586d > targetMinRad && angleRad - 6.283185307179586d <= targetMaxRad)) && dist2(tx, ty) > snapDistance2) {
                                activeTarget = i;
            ...
            if (this.mDragging) {
                ...
                if (this.mActiveTarget != activeTarget) {
                    ...
                    if (activeTarget != -1) {
                        TargetDrawable target2 = targets.get(activeTarget);
                        if (target2.hasState(TargetDrawable.STATE_FOCUSED)) {
                            target2.setState(TargetDrawable.STATE_FOCUSED);
                        }
                        if (this.mMagneticTargets) {
    ...
    public void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        float newWaveCenterX = ((float) this.mHorizontalInset) + (Math.max((float) (right - left), ((float) this.mMaxTargetWidth) + placementWidth) / 2.0f);
        float newWaveCenterY = ((float) this.mVerticalInset) + (Math.max((float) (bottom - top), ((float) this.mMaxTargetHeight) + placementHeight) / 2.0f);
    ...
    private void updateTargetPosition(int i, float centerX, float centerY, float angle) {
        ...
        if (i >= 0) {
            ...
            targetIcon.setX(((float) Math.cos((double) angle)) * placementRadiusX);
            targetIcon.setY(((float) Math.sin((double) angle)) * placementRadiusY);
        }
    }
    ...
    private float getAngle(float alpha, int i) {
        return this.mFirstItemOffset + (((float) i) * alpha);
    }
    ...
    private void announceTargets() {
        ...
        for (int i = 0; i < targetCount; i++) {
            ...
            if (!TextUtils.isEmpty(targetDescription) && !TextUtils.isEmpty(directionDescription)) {
                utterance.append(String.format(directionDescription, targetDescription));
            }
        }
    ...
    public int getTargetPosition(int resourceId) {
        for (int i = 0; i < this.mTargetDrawables.size(); i++) {
            if (this.mTargetDrawables.get(i).getResourceId() == resourceId) {
                return i;
            }
        }
        return -1;
        ...
        if (component != null) {
            try {
                ...
                if (!(metaData == null || (iconResId = metaData.getInt(name)) == 0)) {
                    replaced = replaceTargetDrawables(packageManager.getResourcesForActivity(component), existingResId, iconResId);
                }
